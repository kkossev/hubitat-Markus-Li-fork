 ...
 *  ver. 2.0.0 2023-07-10 (kkossev) - driver name changed to 'Zigbee - Tuya Valve (w/ healthStatus)'; capability 'PresenceSensor' replaced w/ capability 'HealthCheck' (attribute 'healthStatus')
 *
 */

def version() { "2.0.0" } 
def timeStamp() {"2023/07/10 12:19 PM"}
...
metadata {
	definition (name: "Zigbee - Tuya Valve (w/ healthStatus)", namespace: "oh-lalabs.com", author: "Markus Liljergren", filename: "zigbee-tuya-valve", importUrl: "https://raw.githubusercontent.com/kkossev/hubitat-Markus-Li-fork/release/drivers/expanded/zigbee-tuya-valve-expanded.groovy") {
        ...
        fingerprint model:"TS0111", profileId:"0104", endpointId:"01", inClusters:"0000,0003,0004,0005,0006,0702,0B04", outClusters:"0019"
        ...
        fingerprint model:"TS0011", profileId:"0104", endpointId:"01", inClusters:"0000,0004,0005,0006", outClusters:"0019"
        fingerprint model:"TS0001", manufacturer:"_TYZB01_4tlksk8a", profileId:"0104", endpointId:"01", inClusters:"0000,0003,0006", outClusters:"0003,0006,0004", application:"20"
        fingerprint model:"TS0001", profileId:"0104", endpointId:"01", inClusters:"0000,0003,0006", outClusters:"0003,0006,0004", application:"20"
    ...
    preferences {
        ...
        input(name: "invertValve", type: "bool", title: styling_addTitleDiv("Invert open/close"), description: styling_addDescriptionDiv("When open show as closed and vice versa (default: false)"), defaultValue: false)
	}
}
...
String getDeviceInfoByName(infoName) { 
    ...
    Map deviceInfo = ['name': 'Zigbee - Tuya Valve (w/ healthStatus)', 'namespace': 'oh-lalabs.com', 'author': 'Markus Liljergren', 'filename': 'zigbee-tuya-valve', 'importUrl': 'https://raw.githubusercontent.com/kkossev/hubitat-Markus-Li-fork/release/drivers/expanded/zigbee-tuya-valve-expanded.groovy']
...
void configureDevice() {
    ...
    ArrayList<String> cmd = []
    cmd += ["zdo bind 0x${device.deviceNetworkId} ${endpointId} 0x01 0x0006 {${device.zigbeeId}} {}", "delay 187"]
    cmd += ["zdo bind 0x${device.deviceNetworkId} ${endpointId} 0x01 0x0702 {${device.zigbeeId}} {}", "delay 188"]
    sendZigbeeCommands(cmd)
}
 ...
 */
void on() {
    logging("on() valveCommandDirection(0x01)=${valveCommandDirection(0x01)}", 1)
	sendZigbeeCommands(zigbeeCommand(0x006, valveCommandDirection(0x01)))
}

void off() {
    logging("off() valveCommandDirection(0x00)=${valveCommandDirection(0x00)}", 1)
	sendZigbeeCommands(zigbeeCommand(0x006, valveCommandDirection(0x00)))
}

void open() {
    logging("open() valveCommandDirection(0x01)=${valveCommandDirection(0x01)}", 1)
	sendZigbeeCommands(zigbeeCommand(0x006, valveCommandDirection(0x01)))
}

void close() {
    logging("close() valveCommandDirection(0x00)=${valveCommandDirection(0x00)}", 1)
	sendZigbeeCommands(zigbeeCommand(0x006, valveCommandDirection(0x00)))
}

Integer valveCommandDirection(Integer originalCommand) {
    if(invertValve == null) invertValve = false
    return (originalCommand != 0) == invertValve ? 0 : 1
}
