 ...
 *  ver. 2.0.0 2023-07-10 (kkossev) - driver name changed to 'Zigbee - Generic Dimmer (w/ healthStatus)'; capability 'PresenceSensor' replaced w/ capability 'HealthCheck' (attribute 'healthStatus')
 *
 */
...
metadata {
	definition (name: "Zigbee - Generic Dimmer (w/ healthStatus)", namespace: "oh-lalabs.com", author: "Markus Liljergren", filename: "zigbee-generic-dimmer", importUrl: "https://raw.githubusercontent.com/kkossev/hubitat-Markus-Li-fork/release/drivers/expanded/zigbee-generic-dimmer-expanded.groovy") {
    ...
    preferences {
        ...
        input(name: "minLevel", type: "decimal", title: styling_addTitleDiv("Level (minimum)"), description: styling_addDescriptionDiv("Minimum dimming level to send before sending 0, below this level it will be reported as 0. (default: 2)"), defaultValue: "2", range: "0..50")
        input(name: "maxLevel", type: "decimal", title: styling_addTitleDiv("Level (maximum)"), description: styling_addDescriptionDiv("Maximum dimming level to send, at and above this level it will be reported as 100. (default: 97)"), defaultValue: "97", range: "51..100")
	}
}
...
String getDeviceInfoByName(infoName) { 
    ...
    Map deviceInfo = ['name': 'Zigbee - Generic Dimmer (with healthStatus)', 'namespace': 'oh-lalabs.com', 'author': 'Markus Liljergren', 'filename': 'zigbee-generic-dimmer', 'importUrl': 'https://raw.githubusercontent.com/kkossev/hubitat-Markus-Li-fork/release/drivers/expanded/zigbee-generic-dimmer-expanded.groovy']
...
ArrayList<String> refresh() {
    ...
    cmd += ["zdo bind ${device.deviceNetworkId} 0x$endpoint 0x01 0x0006 {${device.zigbeeId}} {}", "delay 187",]
    cmd += ["zdo bind ${device.deviceNetworkId} 0x$endpoint 0x01 0x0008 {${device.zigbeeId}} {}", "delay 188",]
    cmd += zigbee.configureReporting(0x0008, 0x0000, 0x20, 3600, 3600, 1, [:], 189)
    cmd += zigbee.readAttribute(0x0000, 0x0005)
    cmd += zigbee.readAttribute(0x0006, 0x0000)
    cmd += zigbee.readAttribute(0x0008, 0x0000)
...
ArrayList<String> parse(String description) {
    ...
    switch(msgMap["cluster"] + '_' + msgMap["attrId"]) {
        ...
        case "0006_0000":
            ...
            break
        case "0008_0000":
            logging("Dimmer - description:${description} | parseMap:${msgMap}", 100)
            Integer cLevel = Math.round(msgMap['valueParsed'])
            cLevel = cLevel >= deviceMaxLevel() ? 100 : (cLevel < deviceMinLevel() ? 0 : cLevel)
            sendEvent(name:"level", value: cLevel)
...
ArrayList<String> zigbeeSetLevel(Integer endpoint, BigDecimal level, BigDecimal duration, boolean staging=false, boolean full8bitLevel = false) {
  Integer command = staging == false ? 0x04 : 0x00
  duration = command == 0x00 ? 0 : duration
  level = full8bitLevel == true ? Math.round(level * 2.55) : level
  return zigbeeCommand(endpoint, 0x0008, command, 200, "${integerToHexString(level, 1)}${integerToHexString(duration*10, 2, true)}") +  zigbee.readAttribute(0x0008, 0x0000)
}

ArrayList<String> zigbeeStartLevelChange(Integer endpoint, String direction, Integer rate) {
  Integer mode = direction == 'up' ? 0x00 : 0x01
  return zigbeeCommand(endpoint, 0x0008, 0x01, 200, "${integerToHexString(mode, 1)}${integerToHexString(rate, 1)}")
}

ArrayList<String> zigbeeStopLevelChange(Integer endpoint) {
  return zigbeeCommand(endpoint, 0x0008, 0x03, 200)
}

Integer retrieveEndpointId2() {
    ...
    log.info("endpointIdRaw: $endpointIdRaw, endpointId: $endpointId")
