...
metadata {
	definition (name: "Zigbee - Generic Repeater (w/ healthStatus)", namespace: "oh-lalabs.com", author: "Markus Liljergren", filename: "zigbee-generic-repeater", importUrl: "https://raw.githubusercontent.com/kkossev/hubitat-Markus-Li-fork/release/drivers/expanded/zigbee-generic-repeater-expanded.groovy") {
        // BEGIN:getDefaultMetadataCapabilitiesForZigbeeDevices()
        capability "Sensor"
        //capability "PresenceSensor"    // replaced by capability "HealthCheck"
...
String getDeviceInfoByName(infoName) { 
    ...
    Map deviceInfo = ['name': 'Zigbee - Generic Repeater (with healthStatus)', 'namespace': 'oh-lalabs.com', 'author': 'Markus Liljergren', 'filename': 'zigbee-generic-repeater', 'importUrl': 'https://raw.githubusercontent.com/markus-li/Hubitat/release/drivers/expanded/zigbee-generic-repeater-expanded.groovy']
...
void ping() {
    ...
    switch(pingType) {
        case "Disabled":
            ...
            break
        case "Bind":
            cmd += ["zdo bind ${device.deviceNetworkId} 0x01 0x01 0x0000 {${device.zigbeeId}} {}", "delay 187",]
            break
        case null:
        case "Read Attribute":
        default:
    ...
    switch(msgMap["cluster"] + '_' + msgMap["attrId"]) {
        ...
        default:
            switch(msgMap["clusterId"]) {
                ...
                case "8032":
                case "8038":
                    logging("GENERAL CATCHALL (0x${msgMap["clusterId"]}) description:${description}", 100)
                    break
                default:
                    sendlastCheckinEvent(minimumMinutesToRepeat=25)
