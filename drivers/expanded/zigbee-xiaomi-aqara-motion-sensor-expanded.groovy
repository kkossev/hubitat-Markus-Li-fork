...
metadata {
	definition (name: "Zigbee - Xiaomi/Aqara Motion Sensor (w/ healthStatus)", namespace: "oh-lalabs.com", author: "Markus Liljergren", filename: "zigbee-xiaomi-aqara-motion-sensor", importUrl: "https://raw.githubusercontent.com/kkossev/hubitat-Markus-Li-fork/release/drivers/expanded/zigbee-xiaomi-aqara-motion-sensor-expanded.groovy") {
...
String getDeviceInfoByName(infoName) { 
    ...
    Map deviceInfo = ['name': 'Zigbee - Xiaomi/Aqara Motion Sensor (with healthStatus)', 'namespace': 'oh-lalabs.com', 'author': 'Markus Liljergren', 'filename': 'zigbee-xiaomi-aqara-motion-sensor', 'importUrl': 'https://raw.githubusercontent.com/kkossev/hubitat-Markus-Li-fork/release/drivers/expanded/zigbee-xiaomi-aqara-motion-sensor-expanded.groovy']
...
void bindAndRetrieveT1SensorData() {
    ArrayList<String> cmd = []
    String endpoint = '01'
    cmd += ["zdo bind 0x${device.deviceNetworkId} ${endpointId} 0x01 0x0003 {${device.zigbeeId}} {}", "delay 187",]
    cmd += ["zdo bind 0x${device.deviceNetworkId} ${endpointId} 0x01 0x0406 {${device.zigbeeId}} {}", "delay 188",]
...
void configureDevice() {
    ...
    ArrayList<String> cmd = []
    cmd += ["zdo bind 0x${device.deviceNetworkId} ${endpointId} 0x01 0x0000 {${device.zigbeeId}} {}", "delay 189",]
    cmd += ["zdo bind 0x${device.deviceNetworkId} ${endpointId} 0x01 0x0406 {${device.zigbeeId}} {}", "delay 190"]
...
void test() {
  Integer endpointId = 1
    ...
    cmd += zigbeeWriteAttribute(0xFCC0, 0x0102, DataType.UINT8, (Integer) 3, [mfgCode: "0x115F"])
    cmd += zigbee.configureReporting(0x0406, 0x00000, 0x18, 3, 60, null, [:], 50)
...
ArrayList<String> parse(String description) {
    ...
    if(hasCorrectCheckinEvents(maximumMinutesBetweenEvents=90) == false) {
        ...
        restoreCmd += ["zdo bind ${device.deviceNetworkId} 0x01 0x01 0x0406 {${device.zigbeeId}} {}", "delay 72",]
        if(getDeviceDataByName('model') != "lumi.sensor_motion") {
            restoreCmd += ["zdo bind ${device.deviceNetworkId} 0x01 0x01 0x0400 {${device.zigbeeId}} {}", "delay 73",]
        }
